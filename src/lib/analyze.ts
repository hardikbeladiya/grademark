...
export function analyze(startingCapital: number, trades: ITrade[]): IAnalysis {
  if (!isNumber(startingCapital) || startingCapital <= 0) {
    throw new Error(
      "Expected 'startingCapital' argument to 'analyze' to be a positive number that specifies the amount of capital used to simulate trading."
    );
  }

  if (!isArray(trades)) {
    throw new Error(
      "Expected 'trades' argument to 'analyze' to be an array that contains a set of trades to be analyzed."
    );
  }
  ...
  const expectency =
    rmultiples.length > 0 ? new Series(rmultiples).average() : undefined;
  const rmultipleStdDev =
    rmultiples.length > 0 ? std(rmultiples, "unbiased") : undefined;
  ...
  const proportionWinning =
    totalTrades > 0 ? numWinningTrades / totalTrades : 0;
  const proportionLosing = totalTrades > 0 ? numLosingTrades / totalTrades : 0;
  const averageWinningTrade =
    numWinningTrades > 0 ? totalProfits / numWinningTrades : 0;
  const averageLosingTrade =
    numLosingTrades > 0 ? totalLosses / numLosingTrades : 0;
  const analysis: IAnalysis = {
    ...
    expectedValue:
      proportionWinning * averageWinningTrade +
      proportionLosing * averageLosingTrade,
  };
